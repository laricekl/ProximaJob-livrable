  ...
  <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
      <div class="relative flex-1 sm:max-w-sm">
        ...
        <input type="text" name="search" value="{{ $search }}" class="w-full rounded-xl border border-outline-variant/20 bg-white/70 py-2.5 pl-10 pr-4 text-sm transition-all focus:border-secondary-container/50 focus:ring-0" placeholder="Rechercher par nom ou code SCIAN..."/>
      </div>
  ...
  <div class="card-glow rounded-2xl p-5">
    ...
    <form method="POST" action="{{ route('admin.secteurs.store') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-3">
      ...
      <div>
        ...
        <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full rounded-xl border @error('name') border-red-300 @else border-outline-variant/20 @enderror bg-white/70 py-2.5 px-4 text-sm focus:border-secondary-container/50 focus:ring-0" placeholder="Ex: Technologies de l'information" required/>
      ...
      <div>
        ...
        <input type="text" id="scian_code" name="scian_code" value="{{ old('scian_code') }}" class="w-full rounded-xl border border-outline-variant/20 bg-white/70 py-2.5 px-4 text-sm focus:border-secondary-container/50 focus:ring-0" placeholder="Ex: 5415"/>
      </div>
      <div>
        <label for="parent_id" class="block text-xs font-bold uppercase tracking-wider text-outline mb-1">Parent</label>
        <select id="parent_id" name="parent_id" class="w-full rounded-xl border border-outline-variant/20 bg-white/70 py-2.5 px-4 text-sm focus:border-secondary-container/50 focus:ring-0">
            ...
            <tr class="transition-colors hover:bg-surface-container-low/30" x-data="{ editing: false }">
              <td class="px-6 py-3">
                ...
                <form x-show="editing" method="POST" action="{{ route('admin.secteurs.update', $secteur) }}" class="flex flex-wrap items-center gap-2" x-cloak>
                  @csrf @method('PUT')
                  <input type="text" name="name" value="{{ $secteur->name }}" class="w-40 rounded-xl border border-outline-variant/20 bg-white/70 py-1.5 px-3 text-sm focus:border-secondary-container/50 focus:ring-0" required/>
                  <input type="text" name="scian_code" value="{{ $secteur->scian_code }}" class="w-20 rounded-xl border border-outline-variant/20 bg-white/70 py-1.5 px-3 text-sm focus:border-secondary-container/50 focus:ring-0" placeholder="Code"/>
                  <select name="parent_id" class="w-36 rounded-xl border border-outline-variant/20 bg-white/70 py-1.5 px-2 text-sm focus:border-secondary-container/50 focus:ring-0">
