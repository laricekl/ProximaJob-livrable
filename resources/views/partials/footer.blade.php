<footer class="w-full mt-auto border-t border-primary/5 bg-white/55 backdrop-blur-sm">
  <div class="max-w-6xl mx-auto px-4 py-3 md:px-10">
    <div class="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-3 whitespace-nowrap text-xs sm:text-xs text-on-surface-variant">
      <a href="{{ route('welcome') }}" class="opacity-85 hover:opacity-100 transition-opacity shrink-0">
        <x-brand-logo icon-class="w-5 h-auto" text-class="text-xs font-bold font-serif text-primary tracking-tight" />
      </a>
      ...
      <span class="text-outline shrink-0">© {{ date('Y') }} {{ $siteSettings?->site_nom ?? 'ProximaJob' }}.</span>
    </div>
  </div>
</footer>
